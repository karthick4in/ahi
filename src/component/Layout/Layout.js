...
const BreadcrumbLayout = (props) => {
	...
	return (<>
		{H4 != undefined && <div className="row">
					...
					<div className="row">
						<div className="col">
							...
							<ol className="breadcrumb">
								{list.map((item, index) => (
									<li key={index } className="breadcrumb-item"><Link to={item.to}>{item.label}</Link></li>
								))}
								<Link to={ritBtn.to} className="badge bg-primary btn btn-sm  float-right" style={{ position: 'absolute', right: '30px' }}>{ritBtn.label}</Link>
							</ol>
						</div>{/*end col*/}
					</div>{/*end row*/}
